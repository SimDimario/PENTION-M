...
Il lavoro si inserisce nel contesto del progetto \textsc{PENTION} e si concentra in particolare sulla definizione, realizzazione e valutazione dell’artefatto \textsc{PENTION-M}, concepito come sistema \textit{cyber-fisico} basato su simulazione fisica, \textit{Physics-Informed Machine Learning} e pipeline \textit{MLOps} orientate alla tracciabilità forense.

L’obiettivo principale della tesi non è stato quello di sviluppare un singolo modello predittivo, ma di progettare un’architettura coerente e verificabile capace di integrare modelli fisici, algoritmi di apprendimento automatico e requisiti operativi reali all’interno di un unico flusso \textit{end-to-end}.
...
Il primo contributo della tesi riguarda la progettazione di un’architettura \textit{cyber-fisica} modulare per il rilevamento mobile di sostanze chimiche, strutturata in otto layer funzionali.
Tale architettura consente di orchestrare la simulazione dei sensori, l’\textit{ingestion} e la validazione dei dati, l’inferenza dei modelli fisici e PIML, il \textit{monitoring} operativo e la generazione di evidenze forensi verificabili.
...
La costruzione del dataset \textit{PENTION\_EI\_Complete} e il successivo \textit{retraining} dei modelli hanno consentito di ottenere un classificatore più robusto e generalizzabile rispetto alle versioni precedenti, introducendo al contempo meccanismi di calibrazione probabilistica adattiva per il controllo dell’\textit{overconfidence}, in linea con i requisiti operativi delle forze dell’ordine.
...
L’introduzione di \textit{logging} strutturato, \textit{versioning} dei modelli, \textit{monitoring} delle metriche \textit{runtime} e generazione di \textit{bundle} forensi firmati consente di garantire riproducibilità, auditabilità e trasparenza del processo decisionale, aspetti fondamentali per l’utilizzo di sistemi basati su intelligenza artificiale in ambito investigativo.

\section{Impatto scientifico}

Dal punto di vista scientifico, la tesi contribuisce alla letteratura sui sistemi \textit{cyber-fisici} e sul \textit{Physics-Informed Machine Learning} mostrando un’applicazione concreta di tali paradigmi in un dominio forense.
Il lavoro evidenzia come la combinazione di modelli fisici e apprendimento automatico possa essere utilizzata non solo per migliorare l’accuratezza predittiva, ma anche per aumentare la spiegabilità e la coerenza dei risultati, elementi spesso trascurati nei modelli puramente \textit{data-driven}.
...
Pur operando in un contesto simulato, il sistema riflette in modo fedele i \textit{workflow} investigativi reali, integrando misurazioni ambientali, modelli di dispersione, analisi predittive e strumenti di visualizzazione in tempo reale.

L’attenzione alla tracciabilità, alla riproducibilità e alla spiegabilità dei risultati risponde direttamente alle esigenze espresse dagli stakeholder coinvolti nelle attività di \textit{Requirements Engineering}.
In questo senso, il lavoro fornisce una base solida per futuri sviluppi orientati all’adozione operativa, riducendo il divario tra prototipazione accademica e utilizzo reale sul campo.
