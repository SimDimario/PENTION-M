...
\chapter{Contesto, motivazioni e obiettivi della ricerca}

Il presente capitolo delinea il quadro di riferimento della ricerca, introducendo le motivazioni operative e scientifiche che hanno guidato lo sviluppo del sistema \textsc{PENTION-M}.
A partire dall'analisi dello scenario di contrasto alle sostanze sintetiche, vengono definiti gli obiettivi della tesi, il ruolo dell'artefatto all'interno del progetto europeo \textsc{PENTION} e la metodologia di ricerca adottata. Infine, viene fornita una panoramica della struttura complessiva dell'elaborato.
...
\subsection{Sfide operative e tecnologiche affrontabili tramite MLOps}
\label{sec:mlops_challenges}

Alla luce del contesto operativo descritto, il rilevamento e l’interpretazione delle \emph{New Psychoactive Substances} in scenari mobili e urbani pongono una serie di sfide che non possono essere affrontate efficacemente tramite modelli isolati o strumenti analitici tradizionali.
In questa tesi si ipotizza che un approccio basato su \emph{Machine Learning Operations} (MLOps), integrato con modellazione fisica e vincoli forensi, possa contribuire a mitigare tali criticità, almeno in parte.

Le principali sfide individuate sono le seguenti:

\begin{itemize}
    \item \textbf{Variabilità e incertezza dei dati di ingresso.}
    Le misurazioni acquisite sul campo sono soggette a rumore, incompletezza e degrado dovuto a condizioni ambientali non controllate.
    In assenza di un controllo sistematico della qualità del dato, i modelli di inferenza rischiano di produrre risultati instabili o non affidabili.
    In questa tesi si ipotizza che una pipeline MLOps, dotata di meccanismi di validazione, \textit{monitoring} e gestione del \textit{drift}, possa contribuire a rendere il comportamento del sistema più robusto nel tempo.

    \item \textbf{Scollamento tra modelli puramente \textit{data-driven} e vincoli fisici del dominio.}
    Modelli di Machine Learning addestrati esclusivamente sui dati tendono a produrre soluzioni numericamente corrette ma fisicamente implausibili, soprattutto in scenari complessi come la dispersione urbana.
    L’integrazione di modelli fisici e approcci di \emph{Physics-Informed Machine Learning} viene quindi considerata una strategia per ridurre tale divario, migliorando la plausibilità e l’interpretabilità degli output.

    \item \textbf{Instabilità delle predizioni in scenari dinamici e mobili.}
    In un contesto di laboratorio mobile, il sistema deve operare durante il movimento del veicolo, affrontando variazioni rapide delle condizioni ambientali e della geometria osservata.
    Senza un’architettura progettata per l’esecuzione \textit{end-to-end} e il monitoraggio \textit{runtime}, il rischio è quello di ottenere predizioni incoerenti o difficili da confrontare nel tempo.

    \item \textbf{Overconfidence e scarsa calibrazione delle probabilità.}
    In ambito investigativo, una confidenza numerica non calibrata rappresenta un rischio operativo e legale.
    La tesi parte dall’ipotesi che tecniche di calibrazione probabilistica, integrate nella pipeline MLOps e adattate al contesto fisico e computazionale, possano ridurre fenomeni di \textit{overconfidence} tipici dei modelli moderni.

    \item \textbf{Assenza di tracciabilità e auditabilità nei sistemi di analisi automatizzata.}
    Molti strumenti di analisi automatica non forniscono meccanismi nativi per la ricostruzione del processo decisionale.
    In un contesto forense, ciò limita drasticamente l’utilizzabilità dei risultati.
    Questa tesi assume che la progettazione di una pipeline MLOps con \textit{logging} strutturato, \textit{versioning} dei modelli e persistenza dei metadati possa rendere le inferenze verificabili \emph{a posteriori}.

    \item \textbf{Difficoltà di integrazione tra componenti eterogenei.}
    La combinazione di sensori, modelli fisici, modelli ML e interfacce operative è spesso frammentata in soluzioni non interoperabili.
    Un’architettura MLOps modulare e containerizzata viene considerata una possibile risposta a questa frammentazione, favorendo manutenibilità, estendibilità e riusabilità del sistema.
\end{itemize}

Le sfide sopra elencate costituiscono le ipotesi di partenza che guidano la progettazione dell’artefatto \textsc{PENTION-M}.
Nei capitoli successivi, tali problematiche verranno riprese e affrontate in modo progressivo attraverso scelte architetturali, modellistiche e operative, consentendo di valutare in che misura l’approccio MLOps adottato contribuisca effettivamente a mitigarle nel contesto simulato considerato.
