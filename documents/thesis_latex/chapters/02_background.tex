...
\begin{itemize}
	\item \textbf{0}: Cannabinoid analogues;
	\item \textbf{1}: Cathinone analogues;
	\item \textbf{2}: Phenethylamine analogues;
	\item \textbf{3}: Piperazine analogues;
	\item \textbf{4}: Tryptamine analogues;
	\item \textbf{5}: Fentanyl analogues;
	\item \textbf{6}: Other compounds.
\end{itemize}

La distribuzione delle classi nel dataset risulta sbilanciata, in particolare a favore della classe \emph{Other compounds}, rendendo necessaria l’adozione di tecniche di bilanciamento durante l’addestramento.
...
Una sistematizzazione completa del paradigma PIML è fornita dalla survey di~\cite{meng2025piml}, che rappresenta uno dei riferimenti più ampi e recenti sul tema. Gli autori propongono una tassonomia unificata distinguendo tra approcci \emph{physics-for-machine-learning} e \emph{machine-learning-for-physics}, e classificano le modalità di integrazione della conoscenza fisica in tre categorie principali: livello dei dati, livello delle feature e livello della funzione di perdita. La survey introduce inoltre una terminologia condivisa per concetti chiave quali \emph{inductive bias}, \emph{prior physical knowledge} e vincoli fisici \emph{soft} o \emph{hard}, fornendo il quadro concettuale di riferimento utilizzato in questa tesi per l’inquadramento metodologico del PIML.
...
Studi successivi hanno approfondito l’utilizzo delle \emph{Physics-Informed Neural Networks} (PINNs) in fluidodinamica, mostrando come tali approcci siano adatti a problemi di trasporto, diffusione e dispersione in condizioni di osservabilità limitata~\cite{elhassan2025pinnsparse}.
...
L’adozione di modelli di Machine Learning in sistemi operativi richiede pipeline MLOps in grado di gestire versionamento, monitoraggio, drift dei dati e auditabilità~\cite{faubel2023mlops_industry, calefato_secmlops}.
