...
Dal punto di vista operativo, le NPS pongono sfide significative alle forze dell’ordine (LEA) e ai laboratori forensi:
\begin{itemize}
...
Dal punto di vista strumentale, uno spettrometro di massa è composto da tre elementi fondamentali: una sorgente di ionizzazione, un analizzatore di massa e un rivelatore.
Nel caso dell’ionizzazione elettronica, le molecole vengono bombardate con elettroni ad alta energia, producendo ioni molecolari e frammenti caratteristici che vengono successivamente separati in base al rapporto massa/carica.

Il risultato dell’analisi è uno spettro di massa che può essere interpretato come una vera e propria impronta chimica della sostanza analizzata.
Tale impronta risulta particolarmente utile in ambito forense, poiché consente il confronto diretto con librerie di riferimento e l’identificazione automatica tramite tecniche di matching spettrale o modelli di apprendimento automatico.
...
Nel contesto delle NPS, non esiste una singola banca dati esaustiva: i dati disponibili sono distribuiti su più fonti, spesso eterogenee per formato, qualità e convenzioni di \textit{naming}.
...
La libreria SWGDRUG (\emph{Scientific Working Group for the Analysis of Seized Drugs}) rappresenta uno degli standard \textit{de facto} per la spettrometria di massa forense.
...
\begin{itemize}
    \item duplicazione di composti con \textit{naming} differente;
...
Le sostanze nel dataset sono associate a una delle principali classi chimiche NPS considerate nel lavoro, codificate come etichette discrete utilizzate per l’addestramento supervisionato del classificatore.

In particolare, le classi incluse nel dataset sono:
\begin{itemize}
    \item \textbf{0}: \textit{Cannabinoid analogues};
    \item \textbf{1}: \textit{Cathinone analogues};
    \item \textbf{2}: \textit{Phenethylamine analogues};
    \item \textbf{3}: \textit{Piperazine analogues};
    \item \textbf{4}: \textit{Tryptamine analogues};
    \item \textbf{5}: \textit{Fentanyl analogues};
    \item \textbf{6}: \textit{Other compounds}.
\end{itemize}

La distribuzione delle classi nel dataset risulta sbilanciata, in particolare a favore della classe \textit{Other compounds}, rendendo necessaria l’adozione di tecniche di bilanciamento durante l’addestramento.
...
\begin{table}[H]
    ...
    \begin{tabular}{lc}
        \toprule
        \textbf{Classe chimica}  & \textbf{Numero di campioni} \\
        \midrule
        Cathinone analogues      & 620                         \\
        Cannabinoid analogues    & 300                         \\
        Phenethylamine analogues & 402                         \\
        Piperazine analogues     & 111                         \\
        Tryptamine analogues     & 159                         \\
        Fentanyl analogues       & 525                         \\
        Other compounds          & 1225                        \\
        \bottomrule
    \end{tabular}
    \caption{Distribuzione delle classi chimiche nel dataset \texttt{PENTION\_EI\_Complete}.}
    \label{tab:dataset_class_distribution}
\end{table}

Per chiarire la struttura del dataset finale, la Tabella~\ref{tab:pention_ei_example} riporta un estratto rappresentativo della codifica vettoriale degli spettri EI.
...
\begin{table}[H]
    ...
    \resizebox{\textwidth}{!}{%
        \begin{tabular}{lcccccccccccccc}
            \toprule
            \textbf{Sostanza} &
            \textbf{m/z 41}   & \textbf{43}  & \textbf{55}  & \textbf{57}  &
            \textbf{69}       & \textbf{77}  & \textbf{91}  & \textbf{105} &
            \textbf{119}      & \textbf{133} & \textbf{147} & \textbf{161} &
            \textbf{Classe}                                                                                                                                                                                                            \\
            \midrule
            1-(2-Chlorophenyl)piperazine
                              & 0.4          & 2.7          & 1.2          & 0.3  & 0.4 & 10.9  & 2.8  & 0.4 & 1.4 & 0.2  & 0.0  & 0.0   & 3 \\

            1-(2-Methoxyphenyl)piperazine
                              & 15.0         & 46.0         & 25.0         & 10.0 & 8.0 & 135.1 & 50.0 & 8.0 & 1.0 & 5.0  & 4.0  & 0.0   & 3 \\

            1-(2-Cyclopropylethyl)-1H-indole-3-carbonitrile
                              & 6.0          & 30.0         & 34.0         & 2.0  & 6.0 & 116.0 & 12.0 & 9.0 & 3.0 & 39.0 & 24.0 & 116.0 & 6 \\
            \bottomrule
        \end{tabular}%
    }
    \caption{Esempio di rappresentazione vettoriale degli spettri EI nel dataset \texttt{PENTION\_EI\_Complete}.}
    \label{tab:pention_ei_example}
\end{table}
...
Una sistematizzazione completa del paradigma PIML è fornita dalla survey di~\cite{meng2025piml}, che rappresenta uno dei riferimenti più ampi e recenti sul tema. Gli autori propongono una tassonomia unificata distinguendo tra approcci \textit{physics-for-machine-learning} e \textit{machine-learning-for-physics}, e classificano le modalità di integrazione della conoscenza fisica in tre categorie principali: livello dei dati, livello delle feature e livello della funzione di perdita. La survey introduce inoltre una terminologia condivisa per concetti chiave quali \textit{inductive bias}, \textit{prior physical knowledge} e vincoli fisici \textit{soft} o \textit{hard}, fornendo il quadro concettuale di riferimento utilizzato in questa tesi per l’inquadramento metodologico del PIML.
...
In particolare, Kashinath et al. analizzano l’impiego del PIML in contesti atmosferici e climatici, evidenziando come l’integrazione di vincoli fisici migliori stabilità e generalizzazione dei modelli in presenza di fenomeni complessi~\cite{kashinath2021piml}.
Studi successivi hanno approfondito l’utilizzo delle \emph{Physics-Informed Neural Networks} (PINN) in fluidodinamica, mostrando come tali approcci siano adatti a problemi di trasporto, diffusione e dispersione in condizioni di osservabilità limitata~\cite{elhassan2025pinnsparse}.
...
In particolare, l’integrazione della conoscenza fisica avviene a più livelli della pipeline, includendo la generazione dei dati simulati, l’ingegnerizzazione delle feature fisiche e l’introduzione di vincoli nella funzione di perdita dei modelli di apprendimento automatico.
Questa scelta consente di mantenere la coerenza fisica del sistema pur beneficiando della flessibilità dei modelli \textit{data-driven}.

Negli ultimi anni, l’impiego dell’intelligenza artificiale nel dominio delle NPS si è esteso anche a scenari di identificazione in assenza di spettri di riferimento completi.
Approcci basati su modelli generativi e predittivi sono stati proposti per stimare spettri di massa a partire dalla struttura molecolare o per suggerire candidati plausibili quando un composto non è presente nelle librerie esistenti.

Sebbene tali metodologie risultino promettenti in ambito di analisi \textit{offline} o di supporto alla ricerca chimica, il loro utilizzo in contesti operativi sul campo rimane limitato a causa dei requisiti di affidabilità, spiegabilità e tracciabilità richiesti dalle applicazioni forensi.
In PENTION-M, l’uso dell’AI è pertanto orientato a supportare il processo decisionale attraverso modelli fisicamente informati e pipeline controllate, piuttosto che a sostituire le procedure di identificazione forense consolidate.

\section{MLOps in Sistemi Cyber-Fisici e Contesti Forensi}
\label{sec:mlops}

L’adozione di modelli di Machine Learning in sistemi operativi richiede pipeline MLOps in grado di gestire versionamento, monitoraggio, \textit{drift} dei dati e auditabilità~\cite{faubel2023mlops_industry, calefato_secmlops}.
...
Questi limiti costituiscono la motivazione principale per la progettazione di PENTION-M, che estende il sistema verso un paradigma mobile, fisicamente informato e \textit{forense-ready}, come discusso nei capitoli successivi.