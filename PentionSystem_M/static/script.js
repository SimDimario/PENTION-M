...
function showSimCard() {
  simCard.style.display = "block";
}
function hideSimCard() {
  simCard.style.display = "none";
}

function showBundleCard() {
  bundleCard.style.display = "block";
}
function hideBundleCard() {
...
function setStatus(state) {
  ...
  if (state === "patrolling") {
    ...
    statusText.textContent = "Patrolling (moving van)";
  } else if (state === "detected") {
    statusPill.classList.add("status-detected");
    statusText.textContent = "Detection – pipeline in execution";
  } else {
...
function ensureMap() {
  if (!map) {
    map = L.map("map").setView([52.372, 4.9], 13);
...
function renderBundleSummary(bundle) {
  ...
  return `
    ...
    <div class="metric-row">
      ...
      <div class="metric-value">${ev.SensorAir.wind_speed_mps} m/s @ ${
    ev.SensorAir.wind_dir_deg
  }°</div>
    </div>

    <div class="metric-row">
      <div class="metric-label">GPS</div>
      <div class="metric-value">${Number(ev.SensorGPS.latitude).toFixed(
        5
      )}, ${Number(ev.SensorGPS.longitude).toFixed(5)}</div>
    </div>
...
function openReportPopup() {
  ...
  content.innerHTML = `
    ...
    <div class="report-section">
      ...
      <div class="report-grid">
        <div><div class="report-item-label">Simulation ID</div><div class="report-item-value">${safe(
          ev.simulation_id
        )}</div></div>
        <div><div class="report-item-label">Timestamp</div><div class="report-item-value">${safe(
          ev.timestamp
        )}</div></div>
        <div><div class="report-item-label">Bundle name</div><div class="report-item-value">${safe(
          bundle.bundle_name
        )}</div></div>
        <div><div class="report-item-label">Hash SHA256</div><div class="report-item-value">${safe(
          bundle.hash_sha256
        )}</div></div>
      </div>
    </div>
    ...
    <div class="report-section">
      ...
      <div class="report-grid">
        <div><div class="report-item-label">Temperature (°C)</div><div class="report-item-value">${safe(
          air.temperature_C
        )}</div></div>
        <div><div class="report-item-label">Humidity (%)</div><div class="report-item-value">${safe(
          air["humidity_%"]
        )}</div></div>
        <div><div class="report-item-label">Wind</div><div class="report-item-value">${safe(
          air.wind_speed_mps
        )} m/s @ ${safe(air.wind_dir_deg)}°</div></div>
        <div><div class="report-item-label">Stability class</div><div class="report-item-value">${safe(
          air.stability_class
        )}</div></div>
      </div>
    </div>
    ...
    <div class="report-section">
      ...
      <div class="report-grid">
        <div><div class="report-item-label">Compound</div><div class="report-item-value">${safe(
          sub.compound_name
        )}</div></div>
        <div><div class="report-item-label">Noise level</div><div class="report-item-value">${safe(
          sub.noise_level
        )}</div></div>
      </div>
    ...
    <div class="report-section">
      <h3>PIML features</h3>
      <div class="report-grid">
        <div><div class="report-item-label">Sigma_y</div><div class="report-item-value">${safe(
          piml.sigma_y
        )}</div></div>
        <div><div class="report-item-label">Sigma_z</div><div class="report-item-value">${safe(
          piml.sigma_z
        )}</div></div>
        <div><div class="report-item-label">Péclet number</div><div class="report-item-value">${safe(
          piml.pe_number
        )}</div></div>
        <div><div class="report-item-label">Stability index</div><div class="report-item-value">${safe(
          piml.stability_index
        )}</div></div>
      </div>
    </div>
    ...
    <div class="report-section">
      ...
      <div class="report-grid">
        <div><div class="report-item-label">Latitude</div><div class="report-item-value">${safe(
          gps.latitude
        )}</div></div>
        <div><div class="report-item-label">Longitude</div><div class="report-item-value">${safe(
          gps.longitude
        )}</div></div>
        <div><div class="report-item-label">Altitude (m)</div><div class="report-item-value">${safe(
          gps.altitude_m
        )}</div></div>
      </div>
    </div>
    ...
    <div class="report-section">
      <h3>Inference</h3>
      <div class="report-grid">
        <div><div class="report-item-label">Predicted class</div><div class="report-item-value">${safe(
          inf.predicted_class
        )}</div></div>
        <div><div class="report-item-label">Confidence</div><div class="report-item-value">${safe(
          inf.confidence_score
        )}</div></div>
        <div><div class="report-item-label">Dispersion map ID</div><div class="report-item-value">${safe(
          inf.dispersion_map_id
        )}</div></div>
        <div><div class="report-item-label">Predicted source (x,y)</div><div class="report-item-value">${safe(
          inf.predicted_source_location
        )}</div></div>
        <div><div class="report-item-label">Predicted location (lat,lon)</div><div class="report-item-value">${safe(
          inferenceLatLon.latitude
        )}, ${safe(inferenceLatLon.longitude)}</div></div>
      </div>
    </div>
    ...
    <div class="report-section">
      <h3>Monitoring</h3>
      <div class="report-grid">
        <div><div class="report-item-label">Model version</div><div class="report-item-value">${safe(
          mon.model_version
        )}</div></div>
        <div><div class="report-item-label">Latency (ms)</div><div class="report-item-value">${safe(
          mon.latency_ms
        )}</div></div>
        <div><div class="report-item-label">Drift score</div><div class="report-item-value">${safe(
          mon.drift_score
        )}</div></div>
        <div><div class="report-item-label">MSE free</div><div class="report-item-value">${safe(
          mon.mse_free ?? ev.Monitoring?.mse_free
        )}</div></div>
      </div>
    </div>
    ...
    <div class="report-section">
      <h3>PIML Runtime</h3>
      <div class="report-grid">
        <div><div class="report-item-label">Correction – Status</div><div class="report-item-value">${safe(
          pimlRuntime.correction_dispersion_piml?.status
        )}</div></div>
        <div><div class="report-item-label">Correction – Model</div><div class="report-item-value">${safe(
          pimlRuntime.correction_dispersion_piml?.model_version
        )}</div></div>
        <div><div class="report-item-label">Correction – Shape</div><div class="report-item-value">${safe(
          pimlRuntime.correction_dispersion_piml?.corrected_map_shape
        )}</div></div>
        <div><div class="report-item-label">Correction – Hash</div><div class="report-item-value">${safe(
          pimlRuntime.correction_dispersion_piml?.corrected_map_hash
        )}</div></div>
        <div><div class="report-item-label">SourceLoc – Status</div><div class="report-item-value">${safe(
          pimlRuntime.source_localization_piml?.status
        )}</div></div>
        <div><div class="report-item-label">SourceLoc – Model</div><div class="report-item-value">${safe(
          pimlRuntime.source_localization_piml?.model_version
        )}</div></div>
        <div><div class="report-item-label">SourceLoc – XY</div><div class="report-item-value">${safe(
          pimlRuntime.source_localization_piml?.predicted_source_xy
        )}</div></div>
        <div><div class="report-item-label">SourceLoc – Confidence</div><div class="report-item-value">${safe(
          pimlRuntime.source_localization_piml?.confidence
        )}</div></div>
      </div>
    </div>
    ...
    <div class="report-section">
      <h3>ModelOps</h3>
      <div class="report-grid">
        <div><div class="report-item-label">Registry ID</div><div class="report-item-value">${safe(
          modelOps.model_registry_id
        )}</div></div>
        <div><div class="report-item-label">Training data version</div><div class="report-item-value">${safe(
          modelOps.training_data_version
        )}</div></div>
        <div><div class="report-item-label">Retraining trigger</div><div class="report-item-value">${safe(
          modelOps.retraining_trigger
        )}</div></div>
      </div>
    </div>
    ...
    <div class="report-section">
      <h3>Artifacts</h3>
      <div class="report-grid">
        <div><div class="report-item-label">Model hash</div><div class="report-item-value">${safe(
          artifacts.model_hash
        )}</div></div>
        <div><div class="report-item-label">Concentration map hash</div><div class="report-item-value">${safe(
          artifacts.concentration_map_hash
        )}</div></div>
        <div><div class="report-item-label">Training data version</div><div class="report-item-value">${safe(
          artifacts.training_data_version
        )}</div></div>
      </div>
    </div>
    ...
    <div class="report-section">
      ...
      <div class="report-grid">
        <div><div class="report-item-label">Bundle hash</div><div class="report-item-value">${safe(
          bundle.hash_sha256
        )}</div></div>
        <div><div class="report-item-label">Digital signature</div><div class="report-item-value">${safe(
          bundle.signature
        )}</div></div>
        <div><div class="report-item-label">Compliance tags</div><div class="report-item-value">${(
          fexport.compliance_tags || []
        ).join(", ")}</div></div>
      </div>
    </div>
  `;
  ...
  if (spectrum) {
    fullSpectrumChart = renderMassSpectrum("full-spectrum-chart", spectrum, {
      clearExisting: true,
      instanceRef: { value: fullSpectrumChart },
    });
  }
}
...
async function renderPdfMap() {
  return new Promise((resolve, reject) => {
    if (typeof leafletImage !== "function") {
      console.error("leafletImage not available");
      return reject(new Error("leafletImage not available"));
    }
...
async function exportPDF() {
  try {
    ...
    doc.setFontSize(22);
    doc.text("Forensic Detection Report", pageWidth / 2, 150, {
      align: "center",
    });
    doc.setFontSize(14);
    doc.text(
      "Amsterdam Mobile Lab – PIML & MLOps Pipeline",
      pageWidth / 2,
      170,
      { align: "center" }
    );
    ...
    field(
      "Lat/Lon",
      `${safe(ev.inference_latlon?.latitude)}, ${safe(
        ev.inference_latlon?.longitude
      )}`
    );
    section("Monitoring");
  ...
  ws.onmessage = (event) => {
    ...
    if (msg.type === "detection_result") {
      ...
      if (msg.forensic_bundle) {
        ...
        showBundleCard();
        const spec =
          msg.forensic_bundle?.event?.SensorSubstance?.spectrum_ei_1_600 ||
          null;
        if (spec) {
          document.getElementById("mini-spectrum-card").style.display = "block";
          miniSpectrumChart = renderMassSpectrum("mini-spectrum-chart", spec, {
            clearExisting: true,
            instanceRef: { value: miniSpectrumChart },
          });
        }
      }
    }
