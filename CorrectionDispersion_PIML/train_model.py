...
from MCxM_PIML import EarlyStopping
from tqdm import tqdm
from loss_function_piml import physics_masked_loss
    ...
    for epoch in tqdm(range(epochs), desc="Training epochs"):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            mae = torch.mean(
                torch.abs(
                    (output - mc[:, 0])
                    * torch.tensor(binary_map, dtype=torch.float32, device=device)
                )
            )
            loss.backward()
            optimizer.step()
        ...
        print(
            f"[INFO] Epoch {epoch+1}: Train Loss={avg_train_loss:.6f}, Train MAE={avg_train_mae:.6f}"
        )
...
def validate(model, val_loader, binary_mask, device):
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            output = model(mc, wind_features, global_features)
            loss = physics_masked_loss(
                output, mc[:, 0], mask, alpha=1.0, beta=1000.0, gamma=0.1, delta=5.0
            )
