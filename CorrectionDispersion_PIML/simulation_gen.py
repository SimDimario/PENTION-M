...
BINARY_MAP_PATH = os.path.join(
    os.path.dirname(__file__), "binary_maps_data/amsterdam_netherlands_bbox.npy"
)
...
def run_single_simulation(i, csv_date, keep_until=KEEP_NPY_UNTIL):
    ...
    wind_angle_mean = float(
        np.degrees(
            np.arctan2(
                np.mean(np.sin(np.deg2rad(wind_dir))),
...
if __name__ == "__main__":
    ...
    temp_csv_path = os.path.join(
        SAVE_DIR, f"nps_simulated_dataset_gaussiano_{csv_date}_PIML_partial.csv"
    )
    ...
    with ProcessPoolExecutor(max_workers=N_WORKERS) as executor:
        for result in tqdm(
            executor.map(
                run_single_simulation, range(N_SIMULATIONS), [csv_date] * N_SIMULATIONS
            ),
            total=N_SIMULATIONS,
            ...
            if len(results) % SAVE_EVERY == 0:
                ...
                df_partial["gps_x"] = (
                    df_partial["source_x"] / df_partial["source_x"].max()
                )
                df_partial["gps_y"] = (
                    df_partial["source_y"] / df_partial["source_y"].max()
                )
    ...
    csv_path = os.path.join(
        SAVE_DIR, f"nps_simulated_dataset_gaussiano_{csv_date}_PIML.csv"
    )
