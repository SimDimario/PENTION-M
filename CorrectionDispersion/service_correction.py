...
def correct_dispersion(
    ...
    model = load_model(
        building_map, m=m, device=device, pretrained_path=pretrained_path
    )
    cm_agg = np.mean(concentration_map, axis=2)
    mc = torch.tensor(cm_agg, dtype=torch.float32, device=device).unsqueeze(0)
    ...
    if global_feature is not None:
        global_features = torch.tensor(
            global_feature, dtype=torch.float32, device=device
        ).unsqueeze(0)
