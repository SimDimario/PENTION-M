...
for i in range(N_SIMULATIONS):
    ...
    C1, (x, y, z), times, stability, wind_dir, stab_label, wind_label, puff = (
    ...
    for sensor in sensors:
        ...
        row = {
            ...
            "stability_value": stability_value.name,
        ...
        if len(sensor.noisy_concentrations) == 0:
            ...
            for idx, (t_idx, conc) in enumerate(
                zip(sensor.times, sensor.noisy_concentrations)
            ):
                ...
                tsf_records.append(
                    {
                        ...
                        "wind_dir_x": (
                            np.cos(np.deg2rad(wd)) if not sensor.is_fault else np.nan
                        ),
                        "wind_dir_y": (
                            np.sin(np.deg2rad(wd)) if not sensor.is_fault else np.nan
                        ),
...
csv_path = os.path.join(
    SAVE_DIR, f"nps_simulated_dataset_gaussiano_{datetime.datetime.now().date()}.csv"
)
df.to_csv(csv_path, index=False)
df_tsf = pd.DataFrame(tsf_records)
tsf_csv_path = os.path.join(
    SAVE_DIR, f"nps_simulated_dataset_tsfresh_{datetime.datetime.now().date()}.csv"
)
df_tsf.to_csv(tsf_csv_path, index=False)
print(f"\nDataset generated and saved in {csv_path} and {tsf_csv_path}")
