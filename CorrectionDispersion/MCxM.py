...
class EarlyStopping:
    ...
    def __call__(self, val_loss, model):
        if self.best_loss is None or val_loss < self.best_loss - self.min_delta:
            ...
            if self.verbose:
                print(
                    f"[EarlyStopping] No improvement in validation loss for {self.counter} epochs"
                )
...
class MCxM_CNN(torch.nn.Module):
    ...
    def forward(self, gauss_disp, wind_features, global_features):
        ...
        print(
            f"[Forward] Wind features shape: {wind_features.shape if wind_features is not None else None}"
        )
        print(
            f"[Forward] Global features shape: {global_features.shape if global_features is not None else None}"
        )
        ...
        for i in range(self.n_mask_correction):
            ...
            print(
                f"[Forward][Step {i}] After concatenating wind_features, um shape: {um.shape}"
            )
            if global_features is not None:
                um = torch.cat([um, global_features], dim=1)
                print(
                    f"[Forward][Step {i}] After concatenating global_features, um shape: {um.shape}"
                )
            x = um.view(B, -1)
