...
def fix_bbox_order(
    ...
    """
    Ensures that the bounding box is in the correct order: (min_lon, min_lat, max_lon, max_lat).
    """
    if bbox is None:
        return None
    if len(bbox) != 4:
...
def generate_binary_map(
    ...
    """
    Generates a binary map (0 = building, 1 = free space) over an OSM bounding box or area.

    Args:
        place (str): Name of the place (if bbox is None).
        grid_size (int): Number of cells per side of the grid.
        bbox (tuple): Bounding box (min_lon, min_lat, max_lon, max_lat) in EPSG:4326.

    Returns:
        np.ndarray: Binary map (1 = free space, 0 = building).
        dict: Metadata.
    """
    ...
    try:
        if bbox:
            ...
            gdf_bounds = gpd.GeoDataFrame(geometry=[bounds_polygon], crs="EPSG:4326")
            gdf_bounds_proj = gdf_bounds.to_crs(epsg=32633)
            bounds = gdf_bounds_proj.total_bounds
        else:
...
if __name__ == "__main__":
    ...
    quartiere_bbox = (12.478107, 41.894985, 12.495397, 41.903454)
    ...
    if binary_map is not None and binary_map.size > 0:
        output_filename = os.path.join(
            ".",
            "CorrectionDispersion/binary_maps_data",
        ...
        info_text = f"""
            ...
            • Building cells: {building_cells:,}
            • Free cells: {free_cells:,}
            • CRS: {metadata.get('crs', 'N/A')}
            • Resolution: {metadata.get('resolution (m)', 'N/A')} m
            • Building density: {metadata.get('building_density', 'N/A'):.1f}%
            • Mean building height: {metadata.get('mean_height', 'N/A') if metadata.get('mean_height') is not None else 'N/A'} m
