    ...
    if config.dispersion_model == DispersionModelType.PLUME:
        for t in range(len(times)):
            for (x_s, y_s, q, h) in config.stacks:
                C = gauss_func_plume(q, wind_speed[t], wind_dir[t], x_grid, y_grid, z_grid,
        ...
        for t in range(len(times)):
                ...
                for puff in active_puffs:
                    ...
                    puff.x += dx
                    puff.y += dy
                    C = gauss_func_puff(puff, x_grid, y_grid, z_grid, dt, stability[t], wind_speed[t], wind_dir[t])
                    C1[:, :, t] += C

    if config.humidify and config.RH < 0.85:
        C1= apply_hygroscopic_growth(C1, config.RH, config.dry_size, config.aerosol_type)

    return C1, (x_grid, y_grid, z_grid), times, stability, wind_dir, stability_label, wind_label, (puffs if config.dispersion_model == DispersionModelType.PUFF else None)